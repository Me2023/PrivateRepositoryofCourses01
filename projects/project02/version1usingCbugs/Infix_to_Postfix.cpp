...
void InToPost(char A[], char *result) {
    SeqStack OPND; initStack(OPND); Push(OPND, '#'); // 操作符栈初始化，结束符'#'进栈 
    int i = 0, j = 0; // j是A的索引，用于读取；i是result的索引，用于往里储存
    char ch = A[0]; // 开始扫描
    char op;  getTop(OPND, op); // op取栈顶元素
